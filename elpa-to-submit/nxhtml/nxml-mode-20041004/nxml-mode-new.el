...
;; This version of nxml-mode.el have been slightly modified to support
;; nxmlhref.el.  The modifications are small and you find them easily
;; by searching for "href".
;;
;; IMPORTANT: When you compile this file be sure to use the Makefile
;; that comes with nxml.
;;
;; Modifications are done by the author of nxmlhref.el.
...
;; Please see nxmlhref.el for an explanation of the use of href below.

(defface nxml-href-face
  `((((class color) (background light)) (:foreground "blue" :underline t))
    (((class color) (background dark)) (:foreground "cyan" :underline t)))
  "Face used to highlight href attribute."
  :group 'nxml-highlighting-faces)

(defvar nxml-href-keymap nil)


(defun nxml-fontify-attribute (att &optional namespace-declaration)
  (let ((name-face 'nxml-attribute-local-name-face))
    (when nxml-href-keymap
      (when (equal (buffer-substring-no-properties 
		    (xmltok-attribute-name-start att)
		    (xmltok-attribute-name-end att))
		   "href")
	;;(message "found href %s %s %s" (xmltok-attribute-name-start att) (xmltok-attribute-name-colon att) (xmltok-attribute-name-end att) )
	(setq name-face 'nxml-href-face)
	(put-text-property (xmltok-attribute-name-start att)
			   (xmltok-attribute-name-end att)
			   'keymap (symbol-value nxml-href-keymap))
	)
      )
